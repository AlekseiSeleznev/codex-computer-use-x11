...
![Version](https://img.shields.io/badge/version-v0.1.2-84cc16) ![License](https://img.shields.io/badge/license-MIT-blue) ![Rust](https://img.shields.io/badge/Rust-2021-f97316) ![Linux](https://img.shields.io/badge/Linux%20Mint-Cinnamon%2FX11-87cf3e)
...
![X11 Computer Use plugin installed in Codex](assets/codex-plugin-installed.png)

**Release:** `v0.1.2` is the current fresh public baseline: it keeps the standalone plugin, safe installer/uninstaller, source-overlay staging, production-readiness evidence, current OpenSpec specs, rollback-first documentation, a live-verified AT-SPI doctor fix, X11-only doctor readiness with RemoteDesktop/Wayland facts kept debug-only, and the Codex plugin UI screenshot in this README.
...
flowchart LR
  ...
  Core --> Readiness["Doctor readiness\nX11/EWMH baseline only"]
  Core --> Compat["Compatibility/debug facts\nRemoteDesktop, WAYLAND_DISPLAY"]
  ...
  Compat -. "never blocks or degrades X11 readiness" .-> Readiness
  ...
  classDef debug fill:#f8fafc,stroke:#64748b,color:#0f172a,stroke-dasharray: 5 5;
  class Operator actor;
  class CodexHost,Plugin,Tools,Core,State,Evidence,Readiness runtime;
...
- The Rust core owns target resolution, verified-focus gates, root/global X11 coordinates, path-oriented screenshot evidence, and degraded diagnostics for X11-baseline layers.
- Desktop integrations are intentionally thin command/DBus/AT-SPI boundaries with parser tests and controlled fixture evidence; RemoteDesktop/Wayland compatibility facts are debug-only and never block or degrade X11 doctor readiness.
