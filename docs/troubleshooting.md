...
- Existing X11 input paths: `ydotool`, `/tmp/.ydotool_socket`, and `/dev/uinput`.
...
The standalone plugin is an X11/EWMH implementation. `doctor --json` readiness is based on the X11 session, EWMH tools, local input backends, and optional X11 enrichment layers such as screenshot and AT-SPI.

RemoteDesktop portal and Wayland support are outside the current standalone plugin scope. If compatibility facts for RemoteDesktop or `WAYLAND_DISPLAY` remain serialized, treat them as debug-only context: their absence or presence is not a blocker, degraded reason, optional enrichment, unsupported-out-of-scope readiness issue, or recommended next step for the X11 baseline.

Screenshot availability is separate from RemoteDesktop input availability. A working Screenshot method does not prove pointer or keyboard portal input is available, and missing RemoteDesktop input does not degrade X11 readiness.
...
it does not weaken live screenshot-crop output integrity. Static documentation may
use `unsupported_out_of_scope` for product scope, but standalone `doctor --json`
readiness should not emit Wayland or RemoteDesktop portal noise for a passing
X11 baseline.
...
readiness scope. Use an X11 session for production-readiness claims unless a
future OpenSpec change and ADR explicitly add Wayland support; do not fix
RemoteDesktop or Wayland as a `doctor` readiness remediation for this plugin.
