...
evidence rather than as an AT-SPI pass. The fake `busctl` returns a header-only RemoteDesktop introspection response so
compatibility parsing can be checked without contacting the real desktop; this
fact must not create standalone X11 doctor readiness noise. Fake `xdotool` calls are written to
