...
| Target diagnostics compatibility and capability-map `x11-ewmh` facts | backend-upstream | `agent-sh/computer-use-linux` and target `computer-use-linux/src/diagnostics.rs` | standalone doctor JSON evidence plus target-only compatibility fixture evidence | Standalone `doctor` readiness must not require or degrade on RemoteDesktop portal availability |
