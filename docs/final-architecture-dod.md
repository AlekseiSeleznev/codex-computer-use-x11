...
- XDG Desktop Portal Screenshot and RemoteDesktop references were reviewed for target compatibility boundaries; standalone X11 doctor readiness does not require RemoteDesktop portal availability.
...
[
  ...
  },
  {
    ...
    "decision": "Use real X11 readiness vocabulary; screenshot readiness is based on screenshot methods; ydotool readiness requires a connectable socket; RemoteDesktop portal facts are target-compatibility context, not standalone doctor readiness."
  },
  {
...
| input backend works on Cinnamon X11 | yes | pass | `src/doctor.rs`; `src/input.rs`; `src/pointer.rs`; fake e2e xdotool log; target input-backend inspection | If `ydotool`, `/dev/uinput`, or the selected X11 input route is unavailable, targeted input is refused or degraded with reason |
