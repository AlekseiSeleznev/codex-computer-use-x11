...
The lifecycle can run an additional Claude Code reviewer after selected artifacts when `.codex/openspec-claude-review.json` is enabled. The reviewer is invoked through `scripts/openspec-claude-review --change <change> --stage <artifact>`, uses configured model/effort values plus an optional budget cap, and writes structured auxiliary reports under `openspec/changes/<change>/reviews/`. Budget caps are disabled by default (`maxBudgetUsd: null`) and are sent to `claude -p` only when explicitly configured.

If Claude Code reports that the configured budget was exhausted (`error_max_budget_usd` / maximum-budget result), the helper returns a blocking review result and automatically disables Claude review for the current session through `.codex/session/openspec-session.json`. The user must raise or clear the cap and explicitly re-enable review before retrying.

After every reviewer invocation, the assistant must render a user-facing
`## Claude Review` block; raw helper JSON or terminal output is not enough. The
...
- `User-facing questions`: only questions requiring the user's decision. If
  ...
  summarized briefly.
- `Short summary`: verdict, what Claude said, and how the assistant handled the
...
Before any checkpoint that includes a Claude review report, every must-fix,
actionable should-fix, warning, and question must be handled through one of
those dispositions. Unhandled Claude findings block the checkpoint.

This reviewer is optional and never replaces mandatory OpenSpec artifacts, `grill-with-docs`, ADR review, Matt TDD, OpenSpec validation, or git checkpoints. Automated checks use `--validate-config`, `--dry-run`, and fixtures; live `claude -p` calls require local auth and explicit project opt-in.
...
Claude artifact review can also be controlled per session through ignored non-secret `.codex/session/openspec-session.json`, managed by `scripts/openspec-session-state` and the `/opsx:claude-review-*` prompts.
