...
fn gather_system_facts() -> ProbeFacts {
    ...
    let atspi_probe = if atspi_bus_available {
        Some(crate::accessibility::atspi_probe_from_system())
    } else {
        None
    };
    ...
    ProbeFacts {
        ...
        atspi_bus_available,
        atspi_tree_available: atspi_probe
            .as_ref()
            .is_some_and(|probe| probe.tree_available),
        atspi_match_outcome: atspi_probe
            .as_ref()
            .and_then(|probe| probe.match_outcome.clone()),
        atspi_candidate_count: atspi_probe.as_ref().and_then(|probe| probe.candidate_count),
        atspi_controlled_fixture_pass: atspi_probe
            .as_ref()
            .is_some_and(|probe| probe.controlled_fixture_pass),
        uinput_read_write,
    }
}
...
fn accessibility_report(
    ...
    let diagnostic_state = if controlled_fixture_pass {
    ...
    } else if !tree && atspi_bridge_disabled_by_environment(env) {
        ...
        match outcome {
            ...
            "tree_available" => "tree_extraction_available".to_string(),
...
fn atspi_bridge_disabled_by_environment(env: &HashMap<String, String>) -> bool {
    env.get("NO_AT_BRIDGE")
...
fn readiness_report(
    ...
    let optional_enrichments = Vec::new();
    let unsupported_out_of_scope = Vec::new();
    ...
    if !ydotool.ok && !input.abs_pointer.available {
        ...
        blockers_detailed.push(readiness_issue(
            ...
            "environment_limitation",
            detail,
            "Enable /dev/uinput or ydotoold before claiming X11 development input readiness",
        ));
    }

    let can_send_development_input = input.abs_pointer.available || input.ydotool.available;
    let recommended_next_step = if !ewmh.can_query_windows {
        ...
        "Enable a development input backend: /dev/uinput or ydotoold".to_string()
...
mod tests {
    ...
    #[test]
    fn doctor_development_input_uses_local_x11_backends_not_remote_desktop() {
        let mut facts = base_facts();
        facts.uinput_read_write = false;
        facts.ydotool_candidates.clear();
        facts.remote_desktop_introspection = Some(String::new());
        let no_local_input = report_from_probe(facts.clone());
        assert!(!no_local_input.readiness.can_send_development_input);
        assert!(no_local_input
            .readiness
            .recommended_next_step
            .contains("ydotoold"));
        assert!(!no_local_input
        ...
        facts.remote_desktop_introspection = Some("method CreateSession\nmethod SelectDevices\nmethod Start\nmethod NotifyPointerMotion\nproperty AvailableDeviceTypes\n".to_string());
        assert!(
            !report_from_probe(facts.clone())
                .readiness
                .can_send_development_input,
            "RemoteDesktop portal is not a supported development-input backend for the X11-only plugin"
        );
    ...
    #[test]
    fn doctor_x11_taxonomy_ignores_remote_desktop_and_wayland_noise() {
        let mut facts = base_facts();
        facts.remote_desktop_introspection = Some(String::new());
        facts
            .env
        ...
        assert!(
            report.readiness.ok,
            "RemoteDesktop/Wayland compatibility facts must not fail X11 baseline: {:#?}",
            report.readiness
        );
        ...
        assert!(
            report.readiness.degraded_reasons.is_empty(),
            "RemoteDesktop absence and WAYLAND_DISPLAY must not degrade X11 readiness: {:#?}",
            report.readiness
        );
        assert!(report.readiness.degraded_acceptable.is_empty());
        assert!(report
            .readiness
            .optional_enrichments
            .iter()
            .all(|item| item.code != "remote_desktop_portal_unavailable"));
        assert!(report
            .readiness
            .unsupported_out_of_scope
            .iter()
            .all(|item| item.code != "wayland_runtime_out_of_scope"));
        assert!(!report
            .readiness
            .recommended_next_step
            .contains("RemoteDesktop"));
    ...
    #[test]
    fn doctor_remote_desktop_gap_is_neutral_when_x11_input_path_works() {
        let mut facts = base_facts();
        facts.remote_desktop_introspection = Some(String::new());
        let report = report_from_probe(facts);
        assert!(report.readiness.ok, "{:#?}", report.readiness);
        ...
        assert!(
            report.readiness.degraded_reasons.is_empty(),
            "RemoteDesktop portal absence must be neutral for ready X11 baseline: {:#?}",
            report.readiness
        );
        assert!(report
            .readiness
            .optional_enrichments
            .iter()
            .all(|issue| issue.code != "remote_desktop_portal_unavailable"));
        assert!(
            !report
                .readiness
                .recommended_next_step
