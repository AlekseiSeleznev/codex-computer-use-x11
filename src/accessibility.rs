...
pub fn atspi_probe_from_system() -> AtspiProbeReport {
    match collect_atspi_candidates_with_timeout(Some(command_timeout())) {
        Ok(output) if output.ok => {
            let candidate_count = output.candidates.len();
            let tree_available = candidate_count > 0;
            let controlled_fixture_pass = output
                .diagnostics
                .detail
                .as_deref()
                .is_some_and(|detail| detail.contains("controlled_fixture_pass"));
            AtspiProbeReport {
                tree_available: tree_available || controlled_fixture_pass,
                candidate_count: Some(candidate_count),
                match_outcome: Some(if tree_available || controlled_fixture_pass {
                    "tree_available".to_string()
                } else {
                    "no_matching_app_subtree".to_string()
                }),
                controlled_fixture_pass,
                detail: output.diagnostics.detail,
            }
        }
        Ok(output) => AtspiProbeReport {
            tree_available: false,
            candidate_count: Some(output.candidates.len()),
            match_outcome: Some("collector_unavailable".to_string()),
            controlled_fixture_pass: false,
            detail: output.diagnostics.detail,
        },
        Err(error) => AtspiProbeReport {
            tree_available: false,
...
fn collect_atspi_candidates() -> Result<CollectorOutput, String> {
    collect_atspi_candidates_with_timeout(None)
}

fn collect_atspi_candidates_with_timeout(
    timeout: Option<Duration>,
) -> Result<CollectorOutput, String> {
    ...
    if timeout.is_none() {
        ...
        return parse_collector_output(output.status, output.stdout, output.stderr);
    }

    let stdout = child
        .stdout
        .take()
        .ok_or_else(|| "failed to capture AT-SPI collector stdout".to_string())?;
    let stderr = child
        .stderr
        .take()
        .ok_or_else(|| "failed to capture AT-SPI collector stderr".to_string())?;
    ...
    let status = loop {
        match child.try_wait() {
            ...
            Ok(None) if started.elapsed() >= timeout => {
                ...
                return Err("AT-SPI collector timed out".to_string());
            }
    ...
    let stdout = stdout_thread
        .join()
        .map_err(|_| "failed to join AT-SPI collector stdout reader".to_string())??;
    let stderr = stderr_thread
        .join()
        .map_err(|_| "failed to join AT-SPI collector stderr reader".to_string())??;
    parse_collector_output(status, stdout, stderr)
}
...
fn parse_collector_output(
    ...
    serde_json::from_slice::<CollectorOutput>(&stdout)
