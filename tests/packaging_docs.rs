...
fn troubleshooting_docs_cover_degraded_layers_and_drift() {
    ...
    for required in [
        ...
        "xdotool",
        "ydotool",
        "strict RemoteDesktop",
        "empty RemoteDesktop introspection table",
        "Screenshot",
        "AT-SPI",
        "degraded",
        "standalone plugin",
...
fn production_readiness_docs_define_x11_pass_degraded_fail_and_safe_retest() {
    ...
    for required in [
        ...
        "Wayland support and portal-required runtime paths are outside the current X11-only scope",
    ...
    for required in [
        ...
        "--inline-screenshot",
        "unsafe opt-in",
        "RemoteDesktop portal facts are diagnostics only",
        "Wayland support is out of scope",
    ] {
        assert_contains(
            &troubleshooting,
            required,
