...
- `adr/0008-adopt-x11-root-coordinate-model.md` — adopts X11 root/global pixel
  coordinates for X11/EWMH bounds, pointer points, screenshot crop rectangles,
  and app-state composition.
- `adr/0009-adopt-final-cinnamon-x11-v1-dod-baseline.md` — adopts the final
  Cinnamon/X11 v1 Computer Use DoD baseline, including the pass/degraded
  readiness answer and final capability-matrix gate.
- `adr/0010-adopt-x11-provider-takeover-shim.md` — adopts a localized X11
  provider takeover shim for Codex Desktop Linux Computer Use settings while
  preserving standalone plugin identity and bundled rollback.
- `adr/0011-adopt-rollback-first-install-manifest.md` — adopts a rollback-first
  manifest contract for install, uninstall, provider takeover, accessibility
  setup, source-overlay, and live-asset rollback safety.
...
ADR 0008 defines X11 root/global pixel coordinates as the canonical coordinate model for standalone bounds, pointer points, screenshot crop rectangles, and future screenshot/window-context composition. X11 geometry reports must preserve signed optional x/y coordinates, unsigned positive dimensions, and bounds provenance diagnostics when `wmctrl`, `xwininfo`, or provider semantics differ. Standalone screenshot crop smoke may use a validated GNOME Shell-compatible `ScreenshotArea` boundary, but source overlay work should reuse the existing target screenshot provider.
...
ADR 0009 defines the final Cinnamon/X11 v1 Computer Use baseline for this repository. The supported claim is yes for the documented Cinnamon/X11 `x11-ewmh` baseline with listed evidence and degraded diagnostics; Cinnamon Wayland, unstable Cinnamon/Muffin extension work, and unsafe targeted input without verification remain unsupported/out of scope until a future design and ADR change that scope.

ADR 0010 defines the Computer Use settings provider takeover rule: X11 takeover may use a localized settings/provider shim, but it must not globally masquerade `codex-computer-use-x11` as bundled `computer-use`, rewrite bundled plugin ownership, or remove rollback to bundled mode.

ADR 0011 defines the rollback-first install manifest rule: installer-owned mutations across standalone plugin state, accessibility setup, source overlay, provider takeover, and live assets must record non-secret before/after state, distinguish changed vs already-present state, support partial-install rollback, and report drift/blockers instead of blindly overwriting user/admin changes.

Current local validation target: fresh Codex Desktop Linux validation and provider-takeover work now uses `/home/as/Документы/AI_PROJECTS/codex-desktop-linux` (or `CODEX_DESKTOP_LINUX_FULL_PATH` when explicitly set). The older `/home/as/Документы/AI_PROJECTS/codex-desktop-linux-full` path remains historical evidence from earlier rollout attempts, not the default active target. The 2026-06-01 fresh target evidence is recorded under `openspec/changes/archive/2026-06-01-replace-bundled-computer-use-with-x11-provider/reports/fresh-target-install-takeover-20260601T102256Z.json`.
