...
The doctor JSON command MUST determine AT-SPI tree availability from a lightweight accessibility collector probe instead of a hardcoded unavailable value. The report MUST expose the probe facts needed to distinguish a working tree, a missing or disabled bridge, an ambiguous match, and a controlled-fixture pass.
...
Doctor AT-SPI probe behavior MUST be testable through fixtures or a collector seam so the false-negative fix can be verified without requiring the developer's live Cinnamon/X11 session.
