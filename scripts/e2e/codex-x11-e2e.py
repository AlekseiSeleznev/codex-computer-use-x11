...
def uninstall_fake_plugin(codex_home: Path, run_dir: Path, log_path: Path) -> dict[str, Any]:
    env = os.environ.copy()
    env["CODEX_HOME"] = str(codex_home)
    env["CODEX_CONFIG_FILE"] = str(codex_home / "config.toml")
    report = run_dir / "uninstall-plugin-report.json"
    command = [str(REPO_DIR / "scripts/uninstall-codex-plugin.sh"), "--report-json", str(report)]
    result = subprocess.run(
        command,
        cwd=REPO_DIR,
        env=env,
    ...
    (run_dir / "uninstall-plugin.stdout.log").write_text(result.stdout, encoding="utf-8")
    (run_dir / "uninstall-plugin.stderr.log").write_text(result.stderr, encoding="utf-8")
    with log_path.open("a", encoding="utf-8") as log:
        log.write(f"uninstall-codex-plugin exit={result.returncode}\n")
    if result.returncode != 0:
        raise SmokeFailure(
            f"failed to uninstall fake standalone plugin from isolated CODEX_HOME; see {run_dir / 'uninstall-plugin.stderr.log'}"
        )
...
def run_plugin(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        try:
            ...
            mcp.close()
        if temp_root is not None and codex_home is not None:
            uninstall_report = uninstall_fake_plugin(codex_home, run_dir, log_path)
            cache_removed = not (codex_home / "plugins/cache/codex-computer-use-x11").exists()
            marketplace_removed = not (codex_home / "plugins/marketplaces/codex-computer-use-x11").exists()
            config_text = (codex_home / "config.toml").read_text(encoding="utf-8") if (codex_home / "config.toml").is_file() else ""
            config_restored = "codex-computer-use-x11@codex-computer-use-x11" not in config_text and "marketplaces.codex-computer-use-x11" not in config_text
            if not (cache_removed and marketplace_removed and config_restored):
                raise SmokeFailure("fake standalone uninstall did not restore owned plugin state")
            evidence.check(
                "fresh_install_doctor_uninstall",
                "pass",
                "Fake smoke auto-installed the standalone plugin, exercised doctor/MCP routes, then uninstalled and verified owned plugin state was removed",
            ...
            evidence.matrix(
                ...
                "standalone_plugin",
                "pass",
                evidence=["marketplace_metadata", "x11_doctor", "fresh_install_doctor_uninstall"],
            )
