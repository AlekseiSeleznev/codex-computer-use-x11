...
if [ "$INSTALL_PLUGIN" -eq 1 ]; then
    ...
    plugin_cmd=("$SCRIPT_DIR/install-codex-plugin.sh")
...
python3 - "$REPORT_JSON" "$SOURCE_REPORT" "$TARGET" "$PLUGIN_STATUS" "$LIVE_STATUS" "$LIVE_ASSETS_DIR" "$DRY_RUN" <<'PY'
import json
import pathlib
import sys
...
target = sys.argv[3]
plugin_status = sys.argv[4]
live_status = sys.argv[5]
live_assets_dir = sys.argv[6]
...
report = {
    ...
    "target": target,
    "plugin": {"name": "codex-computer-use-x11", "status": plugin_status},
