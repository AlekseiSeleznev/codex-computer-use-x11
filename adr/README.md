...
- `adr/0006-adopt-claude-artifact-review.md` — adds optional Claude Code print-mode artifact review as a Codex-overlay reviewer gate with pinned-model-first stage configuration and structured reports.
- `adr/0007-adopt-automatic-checkpoints-and-claude-session-controls.md` — adopts scoped automatic lifecycle checkpoint commits by default and session-level Claude review controls; ADR 0001 remains otherwise in force except for the narrowed checkpoint-approval rule.
- `adr/0008-adopt-x11-root-coordinate-model.md` — adopts X11 root/global pixel coordinates for X11/EWMH bounds, pointer points, screenshot crop rectangles, and future screenshot/window-context composition with explicit bounds provenance diagnostics.
- `adr/0009-adopt-final-cinnamon-x11-v1-dod-baseline.md` — adopts the final Cinnamon/X11 v1 Computer Use DoD baseline and final capability-matrix gate.
- `adr/0010-adopt-x11-provider-takeover-shim.md` — adopts a localized X11 provider takeover shim for Computer Use settings while preserving standalone plugin identity and bundled rollback.
- `adr/0011-adopt-rollback-first-install-manifest.md` — adopts a rollback-first install manifest contract for cross-surface install/uninstall safety.
...
Those files are the required review/decision/verification gates. `grill.md` records pre-design Matt `grill-with-docs` findings, `design-review.md` records post-design Matt `grill-with-docs` findings, `adr.md` records architectural decision review, and `test-plan.md` records vertical Matt TDD slices and evidence expectations. Optional Claude artifact review reports live under `openspec/changes/<change>/reviews/` as auxiliary reviewer evidence when `.codex/openspec-claude-review.json` enables them; they are not lifecycle artifacts and do not replace mandatory gates. The ADR review must record:
